    ...
    public class BboOrderBookTests
    {
        ...
        public void TestBboBidsMultimapSortCorrect()
        {
            ...
            foreach (var pair in bids)
            {
                ...
                {
                    Assert.AreEqual(Constants.MarketOrderPrice, pair.Key.Price);
                }
                else
                {
                    Assert.AreEqual(expected_order[index].Price, pair.Key.Price);
                }
                Assert.AreEqual(expected_order[index], pair.Value.Order);
                ++index;
            }

            //TODO fix stuff below
            // Should be able to search and find
            //Assert.IsTrue((bids.upper_bound(book::ComparablePrice(true, 1245)))->second.ptr()->price() == 1240);
            //Assert.IsTrue((bids.lower_bound(book::ComparablePrice(true, 1245)))->second.ptr()->price() == 1245);
        }
        ...
        public void TestBboAsksMultimapSortCorrect()
        {
            ...
            //TODO see below
            //Assert.IsTrue((asks.upper_bound(book::ComparablePrice(false, 3235)))->second.ptr()->price() == 3245);
            //Assert.IsTrue((asks.lower_bound(book::ComparablePrice(false, 3235)))->second.ptr()->price() == 3235);
        }
        ...
        public void TestBboAddCompleteBid()
        {
            ...
            // Match - complete
            {
                ...
                Assert.IsTrue(AddAndVerify(order_book, bid1, true, true));
            }
        ...
        public void TestBboAddCompleteAsk()
        {
            ...
            // Match - complete
            {
                ...
                Assert.IsTrue(AddAndVerify(order_book, ask1, true, true));
            }

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(0, 0, 0));
            Assert.IsTrue(dc.VerifyAsk(1251, 1, 100));
        ...
        public void TestBboAddMultiMatchBid()
        {
            ...
            var dc = new DepthCheck(order_book.Depth);
            Assert.IsTrue(dc.VerifyBid(1250, 1, 100));
            Assert.IsTrue(dc.VerifyAsk(1251, 2, 500));
            ...
            // Verify remaining
            Assert.AreEqual(ask1, order_book.Asks.First().Value.Order);
        }
        ...
        public void TestBboAddMultiMatchAsk()
        {
            ...
            // Verify depth
            var dc = new DepthCheck(order_book.Depth);
            Assert.IsTrue(dc.VerifyBid(9250, 2, 600));
            Assert.IsTrue(dc.VerifyAsk(9251, 2, 500));
            ...
            // Match - complete
            {
                ...
                Assert.IsTrue(AddAndVerify(order_book, ask3, true, true));
            }

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(9248, 1, 100));
            Assert.IsTrue(dc.VerifyAsk(9251, 2, 500));
            ...
            // Verify remaining
            Assert.AreEqual(bid2, order_book.Bids.First().Value.Order);
        }
        ...
        public void TestBboAddPartialMatchBid()
        {
            ...
            // Verify depth
            var dc = new DepthCheck(order_book.Depth);
            Assert.IsTrue(dc.VerifyBid(7250, 1, 100));
            Assert.IsTrue(dc.VerifyAsk(7251, 1, 200));

            // Match - partial
            {
                var fc1 = new FillChecker(bid1, 200, 7251 * 200);
                var fc2 = new FillChecker(ask2, 200, 7251 * 200);
                Assert.IsTrue(AddAndVerify(order_book, bid1, true, false));
            }
            ...
            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(7251, 1, 150));
            Assert.IsTrue(dc.VerifyAsk(7252, 1, 100));

            // Verify remaining
            Assert.AreEqual(ask1, order_book.Asks.First().Value.Order);
            Assert.AreEqual(bid1, order_book.Bids.First().Value.Order);
        }
        ...
        public void TestBboAddPartialMatchAsk()
        {
            ...
            // Verify depth
            var dc = new DepthCheck(order_book.Depth);
            Assert.IsTrue(dc.VerifyBid(1251, 1, 350));
            Assert.IsTrue(dc.VerifyAsk(1253, 1, 300));

            // Match - partial
            {
                var fc1 = new FillChecker(ask1, 350, 1251 * 350);
                var fc2 = new FillChecker(bid1, 350, 1251 * 350);
                Assert.IsTrue(AddAndVerify(order_book, ask1, true, false));
            }
            ...
            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1250, 2, 300));
            Assert.IsTrue(dc.VerifyAsk(1251, 1, 50));

            // Verify remaining
            Assert.AreEqual(bid0, order_book.Bids.First().Value.Order);
            Assert.AreEqual(ask1, order_book.Asks.First().Value.Order);
        }
        ...
        public void TestBboAddMultiPartialMatchBid()
        {
            ...
            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1251, 1, 250));
            Assert.IsTrue(dc.VerifyAsk(1252, 1, 100));
        ...
        public void TestBboAddMultiPartialMatchAsk()
        {
            ...
            // Verify depth
            var dc = new DepthCheck(order_book.Depth);
            Assert.IsTrue(dc.VerifyBid(1251, 2, 570));
            Assert.IsTrue(dc.VerifyAsk(1253, 1, 300));
            ...
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1250, 1, 100));
            Assert.IsTrue(dc.VerifyAsk(1251, 1, 130));

            // Verify remaining
            Assert.AreEqual(bid0, order_book.Bids.First().Value.Order);
            Assert.AreEqual((Quantity)100, order_book.Bids.First().Value.OpenQuantity);
            Assert.AreEqual(ask1, order_book.Asks.First().Value.Order);
            Assert.AreEqual((Quantity)130, order_book.Asks.First().Value.OpenQuantity);
        }
        ...
        public void TestBboRepeatMatchBid()
        {
            ...
            // Verify depth
            var dc = new DepthCheck(order_book.Depth);
            Assert.IsTrue(dc.VerifyBid(1251, 1, 900));

            // Match - repeated
            {
                var fc1 = new FillChecker(bid1, 100, 125100);
                var fc2 = new FillChecker(ask0, 100, 125100);
                Assert.IsTrue(AddAndVerify(order_book, ask0, true, true));
            }

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1251, 1, 800));
            ...
            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1251, 1, 500));

            {
                var fc1 = new FillChecker(bid1, 200, 1251 * 200);
                var fc2 = new FillChecker(ask2, 200, 1251 * 200);
                Assert.IsTrue(AddAndVerify(order_book, ask2, true, true));
            }

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1251, 1, 300));

            {
                var fc1 = new FillChecker(bid1, 300, 1251 * 300);
                var fc2 = new FillChecker(ask3, 300, 1251 * 300);
                Assert.IsTrue(AddAndVerify(order_book, ask3, true));
            }
        ...
        public void TestBboRepeatMatchAsk()
        {
            ...
            // Verify depth
            var dc = new DepthCheck(order_book.Depth);
            Assert.IsTrue(dc.VerifyAsk(1251, 1, 900));
            ...
            // Match - repeated
            {
                ...
                Assert.IsTrue(AddAndVerify(order_book, bid0, true, true));
            }

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyAsk(1251, 1, 800));
            ...
            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyAsk(1251, 1, 500));

            {
                var fc1 = new FillChecker(ask1, 200, 1251 * 200);
                var fc2 = new FillChecker(bid2, 200, 1251 * 200);
                Assert.IsTrue(AddAndVerify(order_book, bid2, true, true));
            }

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyAsk(1251, 1, 300));

            {
                var fc1 = new FillChecker(ask1, 300, 1251 * 300);
                var fc2 = new FillChecker(bid3, 300, 1251 * 300);
                Assert.IsTrue(AddAndVerify(order_book, bid3, true));
            }

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1251, 1, 100));
            Assert.IsTrue(dc.VerifyAsk(1252, 1, 100));
        ...
        public void TestBboAddMarketOrderBidMultipleMatch()
        {
            ...
            // Verify depth
            var dc = new DepthCheck(order_book.Depth);
            Assert.IsTrue(dc.VerifyBid(12500, 1, 100));
            Assert.IsTrue(dc.VerifyAsk(12510, 1, 200));
            ...
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(12500, 1, 100));
            Assert.IsTrue(dc.VerifyAsk(0, 0, 0));
        }
        ...
        public void TestBboAddMarketOrderAskMultipleMatch()
        {
            ...
            // Verify depth
            var dc = new DepthCheck(order_book.Depth);
            Assert.IsTrue(dc.VerifyBid(12510, 1, 200));
            Assert.IsTrue(dc.VerifyAsk(12520, 1, 100));
        ...
        public void TestBboMatchMultipleMarketOrderBid()
        {
            ...
            // Match - complete
            {
                ...
                Assert.IsTrue(AddAndVerify(order_book, ask0, true, false));
            }
            ...
            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyAsk(1253, 1, 100));
            Assert.IsTrue(dc.VerifyBid(1250, 1, 100));
        }
        ...
        public void TestBboCancelBid()
        {
            ...
            // Cancel bid
            Assert.IsTrue(CancelAndVerify(order_book, bid0, OrderState.Cancelled));
        ...
        public void TestBboCancelAskAndMatch()
        {
            ...
            // Verify depth
            var dc = new DepthCheck(order_book.Depth);
            Assert.IsTrue(dc.VerifyBid(1250, 2, 200));
            Assert.IsTrue(dc.VerifyAsk(1251, 1, 100));

            // Cancel bid
            Assert.IsTrue(CancelAndVerify(order_book, ask0, OrderState.Cancelled));
        ...
        public void TestBboCancelBidFail()
        {
            ...
            Assert.IsTrue(CancelAndVerify(order_book, bid0, OrderState.Complete));
        ...
        public void TestBboCancelAskFail()
        {
            ...
            // Cancel a filled order
            Assert.IsTrue(CancelAndVerify(order_book, ask0, OrderState.Complete));
        ...
        public void TestBboCancelBidRestore()
        {
            ...
            Assert.IsTrue(AddAndVerify(order_book, ask1, false));
            Assert.IsTrue(AddAndVerify(order_book, ask2, false));
            Assert.IsTrue(AddAndVerify(order_book, ask3, false));
            Assert.IsTrue(AddAndVerify(order_book, ask4, false));
            Assert.IsTrue(AddAndVerify(order_book, ask5, false));
            Assert.IsTrue(AddAndVerify(order_book, ask6, false));
            Assert.IsTrue(AddAndVerify(order_book, ask7, false));
            Assert.IsTrue(AddAndVerify(order_book, ask8, false));
            Assert.IsTrue(AddAndVerify(order_book, ask9, false));
            Assert.IsTrue(AddAndVerify(order_book, ask10, false));
            Assert.IsTrue(AddAndVerify(order_book, bid0, false));
            Assert.IsTrue(AddAndVerify(order_book, bid1, false));
            Assert.IsTrue(AddAndVerify(order_book, bid2, false));
            Assert.IsTrue(AddAndVerify(order_book, bid3, false));
            Assert.IsTrue(AddAndVerify(order_book, bid4, false));
            Assert.IsTrue(AddAndVerify(order_book, bid5, false));
            Assert.IsTrue(AddAndVerify(order_book, bid6, false));
            Assert.IsTrue(AddAndVerify(order_book, bid7, false));
            Assert.IsTrue(AddAndVerify(order_book, bid8, false));
            Assert.IsTrue(AddAndVerify(order_book, bid9, false));
            Assert.IsTrue(AddAndVerify(order_book, bid10, false));
            Assert.IsTrue(AddAndVerify(order_book, bid11, false));
            Assert.IsTrue(AddAndVerify(order_book, bid12, false));

            // Verify sizes
            Assert.AreEqual(13, order_book.Bids.Count());
            Assert.AreEqual(11, order_book.Asks.Count());

            // Verify depth
            var dc = new DepthCheck(order_book.Depth);
            Assert.IsTrue(dc.VerifyBid(1249, 3, 500));
            Assert.IsTrue(dc.VerifyAsk(1250, 1, 500));

            // Cancel a bid level (erase)
            Assert.IsTrue(CancelAndVerify(order_book, bid3, OrderState.Cancelled));
            ...
            Assert.IsTrue(CancelAndVerify(order_book, bid7, OrderState.Cancelled));
            Assert.IsTrue(CancelAndVerify(order_book, bid4, OrderState.Cancelled));
            ...
            Assert.IsTrue(CancelAndVerify(order_book, bid1, OrderState.Cancelled));
            Assert.IsTrue(CancelAndVerify(order_book, bid0, OrderState.Cancelled));
            Assert.IsTrue(CancelAndVerify(order_book, bid2, OrderState.Cancelled));

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1246, 1, 500));
            Assert.IsTrue(dc.VerifyAsk(1250, 1, 500));
        }
        ...
        public void TestBboCancelAskRestore()
        {
            ...
            Assert.IsTrue(CancelAndVerify(order_book, ask1, OrderState.Cancelled));
            ...
            Assert.IsTrue(CancelAndVerify(order_book, ask2, OrderState.Cancelled));
            Assert.IsTrue(CancelAndVerify(order_book, ask6, OrderState.Cancelled));
            ...
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1249, 3, 500));
            Assert.IsTrue(dc.VerifyAsk(1252, 1, 200));
        }
        ...
        public void TestBboFillCompleteBidRestoreDepth()
        {
            ...
            var cross_ask = new SimpleOrder(false, 1249, 800);
            {
                ...
                Assert.IsTrue(AddAndVerify(order_book, cross_ask, true, false));
            }

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1248, 1, 400));
            Assert.IsTrue(dc.VerifyAsk(1249, 1, 300)); // Inserted
            ...
            var cross_ask2 = new SimpleOrder(false, 1248, 400);
            {
                ...
                Assert.IsTrue(AddAndVerify(order_book, cross_ask2, true, true));
            }

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1246, 2, 1100));
            Assert.IsTrue(dc.VerifyAsk(1249, 1, 300));
            ...
            var cross_ask3 = new SimpleOrder(false, 1246, 2400);
            {
                ...
                Assert.IsTrue(AddAndVerify(order_book, cross_ask3, true, false));
            }

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1245, 3, 500));
            Assert.IsTrue(dc.VerifyAsk(1246, 1, 1300));
            ...
            var cross_ask4 = new SimpleOrder(false, 1245, 250);
            {
                ...
                Assert.IsTrue(AddAndVerify(order_book, cross_ask4, true, true));
            }

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1245, 2, 250)); // 1 filled, 1 reduced
            Assert.IsTrue(dc.VerifyAsk(1246, 1, 1300));
        }
        ...
        public void TestBboFillCompleteAskRestoreDepth()
        {
            ...
            var cross_bid = new SimpleOrder(true, 1250, 800);
            {
                ...
                Assert.IsTrue(AddAndVerify(order_book, cross_bid, true, false));
            }

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1250, 1, 300));
            Assert.IsTrue(dc.VerifyAsk(1251, 1, 400));
            ...
            var cross_bid2 = new SimpleOrder(true, 1251, 400);
            {
                ...
                Assert.IsTrue(AddAndVerify(order_book, cross_bid2, true, true));
            }

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1250, 1, 300));
            Assert.IsTrue(dc.VerifyAsk(1252, 2, 300));
            ...
            var cross_bid3 = new SimpleOrder(true, 1252, 2400);
            {
                ...
                Assert.IsTrue(AddAndVerify(order_book, cross_bid3, true, false));
            }

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1252, 1, 2100)); // Insert
            Assert.IsTrue(dc.VerifyAsk(1254, 1, 300));
            ...
            var cross_bid4 = new SimpleOrder(true, 1254, 300);
            {
                ...
                Assert.IsTrue(AddAndVerify(order_book, cross_bid4, true, true));
            }

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1252, 1, 2100));
            Assert.IsTrue(dc.VerifyAsk(1255, 2, 700));
            ...
            var cross_bid5 = new SimpleOrder(true, 1255, 550);
            {
                ...
                Assert.IsTrue(AddAndVerify(order_book, cross_bid5, true, true));
            }

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1252, 1, 2100));
            Assert.IsTrue(dc.VerifyAsk(1255, 1, 150)); // 1 filled, 1 reduced
        }
        ...
        public void TestBboReplaceSizeDecrease()
        {
            ...
            var dc = new DepthCheck(order_book.Depth);
            Assert.IsTrue(dc.VerifyBid(1251, 1, 100));
            Assert.IsTrue(dc.VerifyAsk(1252, 2, 500));

            // Verify changed stamps
            Assert.IsTrue(cc.VerifyBboChanged(true, true));
            cc.Reset();

            // Replace size
            Assert.IsTrue(ReplaceAndVerify(order_book, bid0, -60));
            Assert.IsTrue(ReplaceAndVerify(order_book, ask0, -150));

            // Verify orders
            Assert.AreEqual((Quantity)40, bid0.OrderQty);
            Assert.AreEqual((Quantity)150, ask0.OrderQty);

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1251, 1, 100));
            Assert.IsTrue(dc.VerifyAsk(1252, 2, 350));

            // Verify changed stamps
            Assert.IsTrue(cc.VerifyBboChanged(false, true));
        }
        ...
        public void TestBboReplaceSizeDecreaseCancel()
        {
            ...
            // Verify depth
            var dc = new DepthCheck(order_book.Depth);
            Assert.IsTrue(dc.VerifyBid(1251, 1, 400));
            Assert.IsTrue(dc.VerifyAsk(1252, 2, 500));
...

            {
                ...
                Assert.IsTrue(AddAndVerify(order_book, cross_bid, true, true));
            }
            {
                var fc1 = new FillChecker(cross_ask, 100, 1251 * 100);
                var fc2 = new FillChecker(bid1, 100, 1251 * 100);
                Assert.IsTrue(AddAndVerify(order_book, cross_ask, true, true));
            }

            // Verify quantity
            Assert.AreEqual((Quantity)175, ask0.OpenQuantity);
            Assert.AreEqual((Quantity)300, bid1.OpenQuantity);

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1251, 1, 300));
            Assert.IsTrue(dc.VerifyAsk(1252, 2, 375));
            ...
            // Verify orders
            Assert.AreEqual((Quantity)125, ask0.OrderQty);
            Assert.AreEqual((Quantity)0, ask0.OpenQuantity);
            ...
            // Verify orders
            Assert.AreEqual((Quantity)300, bid1.OrderQty);
            Assert.AreEqual((Quantity)200, bid1.OpenQuantity);

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1251, 1, 200));
            Assert.IsTrue(dc.VerifyAsk(1252, 1, 200));
            ...
            // Verify orders
            Assert.AreEqual((Quantity)100, bid1.OrderQty);
            Assert.AreEqual((Quantity)0, bid1.OpenQuantity);
        ...
        public void TestBboReplaceSizeDecreaseTooMuch()
        {
            ...
            Assert.AreEqual((Quantity)100, ask0.OpenQuantity);
            ...
            // Verify ask0 state
            Assert.AreEqual((Quantity)0, ask0.OpenQuantity);
            Assert.AreEqual((Quantity)200, ask0.OrderQty);
            Assert.AreEqual(OrderState.Cancelled, ask0.State);
        ...
        public void TestBboReplaceSizeIncreaseDecrease()
        {
            ...
            // Replace size
            Assert.IsTrue(ReplaceAndVerify(order_book, ask0, 50));
            Assert.IsTrue(ReplaceAndVerify(order_book, bid0, 25));

            Assert.IsTrue(ReplaceAndVerify(order_book, ask0, -100));
            Assert.IsTrue(ReplaceAndVerify(order_book, bid0, 25));

            Assert.IsTrue(ReplaceAndVerify(order_book, ask0, 300));
            Assert.IsTrue(ReplaceAndVerify(order_book, bid0, -75));

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1250, 1, 75));
            Assert.IsTrue(dc.VerifyAsk(1251, 1, 550));
        }
        ...
        public void TestBboReplaceBidPriceChange()
        {
            ...
            // Verify depth
            var dc = new DepthCheck(order_book.Depth);
            Assert.IsTrue(dc.VerifyBid(1251, 1, 140));
            Assert.IsTrue(dc.VerifyAsk(1252, 1, 200));

            // Replace price increase
            Assert.IsTrue(ReplaceAndVerify(order_book, bid0, Constants.SizeUnchanged, 1251));

            Assert.AreEqual((Price)1251, order_book.Bids.First().Key.Price);
            Assert.AreEqual(bid1, order_book.Bids.First().Value.Order);
            Assert.AreEqual((Price)1251, order_book.Bids.ElementAt(1).Key.Price);
            Assert.AreEqual(bid0, order_book.Bids.ElementAt(1).Value.Order);
            Assert.AreEqual(2, order_book.Bids.Count());

            // Verify order
            Assert.AreEqual((Price)1251, bid0.Price);
            Assert.AreEqual((Quantity)120, bid0.OrderQty);

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1251, 2, 260));
            Assert.IsTrue(dc.VerifyAsk(1252, 1, 200));

            // Replace price decrease
            Assert.IsTrue(ReplaceAndVerify(order_book, bid1, Constants.SizeUnchanged, 1250));

            // Verify price change in book
            Assert.AreEqual((Price)1251, order_book.Bids.First().Key.Price);
            Assert.AreEqual(bid0, order_book.Bids.First().Value.Order);
            Assert.AreEqual((Price)1250, order_book.Bids.ElementAt(1).Key.Price);
            Assert.AreEqual(bid1, order_book.Bids.ElementAt(1).Value.Order);
            Assert.AreEqual(2, order_book.Bids.Count());

            // Verify order
            Assert.AreEqual((Price)1250, bid1.Price);
            Assert.AreEqual((Quantity)140, bid1.OrderQty);

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1251, 1, 120));
            Assert.IsTrue(dc.VerifyAsk(1252, 1, 200));
        }
        ...
        public void TestBboReplaceAskPriceChange()
        {
            ...
            Assert.IsTrue(ReplaceAndVerify(order_book, ask1, Constants.SizeUnchanged, 1253));

            // Verify price change in book
            Assert.AreEqual((Price)1253, order_book.Asks.First().Key.Price);
            Assert.AreEqual(ask0, order_book.Asks.First().Value.Order);
            Assert.AreEqual((Price)1253, order_book.Asks.ElementAt(1).Key.Price);
            Assert.AreEqual(ask1, order_book.Asks.ElementAt(1).Value.Order);
            Assert.AreEqual(2, order_book.Asks.Count());

            // Verify order
            Assert.AreEqual((Price)1253, ask1.Price);
            Assert.AreEqual((Quantity)200, ask1.OrderQty);

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1251, 1, 140));
            Assert.IsTrue(dc.VerifyAsk(1253, 2, 500));

            // Replace price decrease 1253 -> 1252
            Assert.IsTrue(ReplaceAndVerify(order_book, ask0, Constants.SizeUnchanged, 1252));

            // Verify price change in book
            Assert.AreEqual((Price)1252, order_book.Asks.First().Key.Price);
            Assert.AreEqual(ask0, order_book.Asks.First().Value.Order);
            Assert.AreEqual((Price)1253, order_book.Asks.ElementAt(1).Key.Price);
            Assert.AreEqual(ask1, order_book.Asks.ElementAt(1).Value.Order);
            Assert.AreEqual(2, order_book.Asks.Count());
            ...
            // Verify order
            Assert.AreEqual((Price)1252, ask0.Price);
            Assert.AreEqual((Quantity)300, ask0.OrderQty);

            // Verify depth
            dc.Reset();
            Assert.IsTrue(dc.VerifyBid(1251, 1, 140));
            Assert.IsTrue(dc.VerifyAsk(1252, 1, 300));
        }
    }
}